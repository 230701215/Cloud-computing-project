/**
 * Express API for Azure Blob Storage + Firebase Auth - FINAL VERSION
 */
...
import fs from 'node:fs/promises';   // Use promise version to avoid require
...
// Check if dist exists asynchronously
fs.access(DIST_DIR).then(() => {
  console.log('dist folder exists: true');
}).catch(() => {
  console.log('dist folder exists: false');
});
...
app.use((req, res) => {
  ...
  res.sendFile(path.join(DIST_DIR, 'index.html'), (err) => {
    if (err) {
      ...
      res.status(500).send('React build not found. Run "npm run build" and push again.');
    }
  });
});
